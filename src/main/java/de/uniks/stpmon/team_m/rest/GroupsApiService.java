...
public interface GroupsApiService {
    ...
    @PATCH("groups/{id}")
    Observable<Group> update(@Path("id") String ignoredIgnoredIgnoredIgnored_id, @Body UpdateGroupDto ignoredDto);

    @DELETE("groups/{id}")
    Observable<Group> delete(@Path("id") String ignoredIgnoredIgnoredIgnored_id);
}
